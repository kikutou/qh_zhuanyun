...
/*					//推荐人积分计算
					...
					foreach($all___warehouse__lists as $home){
						...
						if($home['xunistorage']==0 && $addrs['addresstype']==1){
					...
					if($member_setting['choosemodel']) {	//如果开启选择模型
						...
						$user_model_info =array();// $member_input->get($_POST['info']);
						$user_model_info['userid'] = $userid;
						$user_model_info['firstname'] = $firstname;
						$user_model_info['lastname'] = $lastname;
						$user_model_info['truename'] = $lastname.$firstname;
					...
					if($member_setting['enablemailcheck']) {
						...
						$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'index.php?m=member&c=index&a=init';
	...
	public function account_manage_password() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['info']['newpassword']!=$_POST['info']['renewpassword']){
				showmessage("新密码与确认密码不一致", HTTP_REFERER);
			}
			//修改会员邮箱
...
/*			if(pc_base::load_config('system', 'phpsso')) {
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			*/
			...
			param::set_cookie('_clientcode', $_clientcode, $cookietime);
			param::set_cookie('_clientname', $_clientname, $cookietime);


			//param::set_cookie('cookietime', $_cookietime, $cookietime);
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'index.php?m=member&c=index';
			if($synloginstr=='0'){$synloginstr='';}
			showmessage(L('login_success').$synloginstr, 'index.php?m=member&c=index');
		} else {
	...
	public function change_credit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$this->db->update(array('amount'=>"+=$point"), array('userid'=>$memberinfo['userid']));
			//加入消费记录，同时扣除金钱
			pc_base::load_app_class('spend','pay',0);
			spend::point($money, L('buy_point'), $memberinfo['userid'], $memberinfo['username']);
			showmessage(L('operation_success'), HTTP_REFERER);
		} else {
