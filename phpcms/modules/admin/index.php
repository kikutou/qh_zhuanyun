...
class index extends admin {
	...
	public function login() {
		if(isset($_GET['dosubmit'])) {
			...
			if($r['password'] != $password) {
				...
				showmessage(L('password_error',array('times'=>$times)),'?m=admin&c=index&a=login',86400);
			}
			...
			$this_pdb->query("UPDATE t_package  SET status='18' where operatorid>0 AND (operatorid+".$_over_time2_.")<".SYS_TIME." and status=2  order by addtime desc ");

			$psql="select * from t_package where operatorid>0 AND (operatorid+".$_over_time_.")<".SYS_TIME." and status=2 order by addtime desc ";
			...
			**/
			...
			$sn = urlencode($_siteinfos['name']);
			$dm = urlencode($_siteinfos['domain']);
			$synclo = str_replace('sn=','sn='.$sn,str_replace('dm=','dm='.$dm,SYS_REFERER));
			$this->db->update(array('lastloginip'=>ip(),'lastlogintime'=>SYS_TIME),array('userid'=>$r['userid']));
			$_SESSION['userid'] = $r['userid'];
			$_SESSION['roleid'] = $r['roleid'];
			$_SESSION['checkhash'] = random(6,'abcdefghigklmnopqrstuvwxwyABCDEFGHIGKLMNOPQRSTUVWXWY0123456789');
			...
			showmessage(L('login_success').$synclo,'?m=admin&c=index');
			//同步登陆vms,先检查是否启用了vms
	...
	public function public_ajax_add_panel() {
		...
		foreach($panelarr as $v) {
			echo "<span><a onclick='paneladdclass(this);' target='right' href='".$v['url'].'&menuid='.$v['menuid']."&checkhash=".$_SESSION['checkhash']."'>".L($v['name'])."</a>  <a class='panel-delete' href='javascript:delete_panel(".$v['menuid'].");'></a></span>";
		}
		exit;
	}
	...
	public function public_ajax_delete_panel() {
		...
		$panelarr = $this->panel_db->listinfo(array('userid'=>$_SESSION['userid']), "datetime");
		foreach($panelarr as $v) {
			echo "<span><a onclick='paneladdclass(this);' target='right' href='".$v['url']."&checkhash=".$_SESSION['checkhash']."'>".L($v['name'])."</a> <a class='panel-delete' href='javascript:delete_panel(".$v['menuid'].");'></a></span>";
		}
		exit;
	}
